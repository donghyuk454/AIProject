...
class Naive_Bayes(AI_util):
    ...
    def predict(self, test_data: list):
        ...
        ### EDIT FROM HERE ###
        labelList = ['finance', 'lifestyle', 'tv', 'sports', 'entertainment']

        for docu in test_data:
            document = docu[0]
            id_ = docu[2]
            tfVector = self.get_tf_vector(self, tfVector)
            vecLen = len(tfVector)
            tempValues = [0,0,0,0,0] # f, l, t, s, e
    ...
    def calc_conditional_prob(self, train_data: list):
        ...
        for document in train_data:
            ...
            label = document[1]
            id_ = document[2]
            tfVector = self.get_tf_vector(self, document[0]) # get tf vector
            tfVectorLen = len(tfVector)

            for i in range(0, 5):
                if labelList[i] == label:
                    for tf in tfVector:
                        labelTokenLen[i] += float(tf) + alpha
        ...
        for tfL in tfVectorList:
            tfVector = tfVectorList[1]
            label_ = tfVectorList[2]
            for i in range(0, 5):
                if label == labelList[i]:
                    for j in range(0, tfVectorLen):
                        cpVectorList[i][0][j] += float(tfVector[j]) + alpha
        
        for i in range(0, 5):
            for j in range(0, tfVectorLen):
                cpVectorList[i][0][j] /= labelTokenLen[i]
...
def main(args, logger):
    ...
    with open(args.output_dir/'{}_{}.txt'.format(args.std_name, args.std_id), 'w', encoding='utf-8') as f:
        ...
        for i in range(0, length):
            if preds[i] == "finance":
                f.write("{}%\t{}\t{}\n".format(str(ids[i]), str(preds[i]), str(values[i])))
